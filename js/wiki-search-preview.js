 ...
 * Sidebar search: live preview dropdown as you type.
...
(function () {
  ...
  var input   = document.getElementById('sbSearchInput');
  var preview = document.getElementById('sbSearchPreview');
  ...
  var MAX_ITEMS  = 6;
  var debounceT  = null;
  var SEARCH_URL = input.closest('.sb-search') ?
    (document.querySelector('meta[name="search-url"]') ?
      document.querySelector('meta[name="search-url"]').content :
      '/wiki/search/') :
    '/wiki/search/';

  /* Try to derive the search base URL from the form's action or fallback */
  var form = input.closest('form');
  if (form) SEARCH_URL = form.action || SEARCH_URL;
  ...
  function runPreview(q) {
    ...
    // Score: title match = 3, description = 2, content = 1
